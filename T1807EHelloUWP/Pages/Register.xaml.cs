    ...
    public sealed partial class Register : Page
    {
        ...
        private void ButtonBase_OnClick(object sender, RoutedEventArgs e)
        {
            Member member = new Member
            {
                ...
                birthday = TargetInstallDate.Date.ToString("yyyy-MM-dd"),
                email = txtEmail.Text,
                gender = ((nam.IsChecked == true) ? 1 : 2),
                introduction = "Hello, " + txtLastName.Text,
                phone = txtPhoneNumber.Text
            };
            // validate phía client.
            Debug.WriteLine(JsonConvert.SerializeObject(member));

            var returns = memberService.Register(member);
            if (returns == null)
            {
                // show error
            }
